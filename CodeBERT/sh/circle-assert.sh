...
train_file=$data_dir/$lang/train_ssl_processed.jsonl
dev_file=$data_dir/$lang/valid_processed.jsonl
unlabel_file=$data_dir/$lang/unlabel_processed.jsonl
...
do 
  ...
  python run_aug.py --do_train  --model_type roberta --model_name_or_path $pretrained_model --lang $lang \
                    ...
                    --mode $mode --unlabel_filename $unlabel_file  --pseudo_filename $pseudo_file  --label_filename $label_file  --mlm_probability 0.1  --submode 'build'
  #retrain model 
  out_dir=checkpoint-topk-$threshold-mlm-0.1-idx-$i
  pseudo_file=pseudo-$lang-$i-all.jsonl
  ...
  python run_aug.py --do_train --do_eval --model_type roberta --model_name_or_path $pretrained_model --lang $lang \
                    ...
                    --mode $mode --unlabel_filename $unlabel_file --threshold $threshold --pseudo_filename $pseudo_file --selected_pseudo_filename $selected_pseudo_file --out_dir $out_dir --mlm_probability 0.1


  #Generate
  test_model=$output_dir/$out_dir/pytorch_model.bin #checkpoint for test
  test_file=$data_dir/$lang/test_processed.jsonl
  python run_aug.py --do_eval --do_test --model_type roberta --model_name_or_path $pretrained_model  --lang $lang \
                    ...
                    --max_source_length $source_length --max_target_length $target_length --beam_size $beam_size --eval_batch_size $eval_batch_size --out_dir $out_dir --mlm_probability 0.1
  origin_dir=$out_dir
