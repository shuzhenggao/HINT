...
class Model(nn.Module):   
    ...
    def forward(self, input_ids=None,labels=None,score=None,input_ids_aug=None): 
        ...
        if labels is not None:
            if 'pl_ours' in self.args.mode:
                ...
                outputs_aug = self.encoder(input_ids=input_ids_aug, attention_mask=attention_mask,
                                       labels=input_ids_aug, decoder_attention_mask=attention_mask, output_hidden_states=True)
                hidden_states_aug = outputs_aug['decoder_hidden_states'][-1]
                vec_aug = hidden_states_aug[eos_mask, :].view(hidden_states_aug.size(0), -1,
                                                      hidden_states_aug.size(-1))[:, -1, :]
                logits_aug = self.classifier(vec_aug)
