...
from transformers import (WEIGHTS_NAME, AdamW, get_linear_schedule_with_warmup,
                          ...
                          DistilBertConfig, DistilBertForMaskedLM, DistilBertTokenizer, T5Config, T5ForConditionalGeneration)
from utils import InputFeatures, convert_examples_to_features, TextDataset, set_seed
...
MODEL_CLASSES = {
    ...
    'roberta': (T5Config, T5ForConditionalGeneration, RobertaTokenizer),
    'distilbert': (DistilBertConfig, DistilBertForMaskedLM, DistilBertTokenizer)
}
#   'roberta': (RobertaConfig, RobertaModel, RobertaForSequenceClassification, RobertaTokenizer),
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        ...
        for step, batch in enumerate(bar):
            ...
            scores=batch[2].to(args.device) 
            inputs_aug = inputs.clone().detach()
            
            aug_idx = random.randint(0,3)
            if aug_idx%4 == 0:
                inputs_aug[:, 2:], _ = mask_tokens(inputs.clone().detach()[:, 2:] ,tokenizer,0.1)
            elif aug_idx%4 == 1:
                code_types = inputs.clone().detach()
                inputs_aug[:, 2:], _ = replace_with_type_tokens(inputs.clone().detach()[:, 2:], code_types.clone()[:, 2:],tokenizer,0.1)
            elif aug_idx%4 == 2:
                choice_token_id  = choice(special_token_id_list)
                inputs_aug[:, 2:], _ = replace_special_token_with_type_tokens(inputs.clone().detach()[:, 2:], choice_token_id, tokenizer,0.1)
            elif aug_idx%4 == 3:
                choice_token_id  = choice(special_token_id_list)
                inputs_aug[:, 2:], _ = replace_special_token_with_mask(inputs.clone().detach()[:, 2:], choice_token_id,tokenizer, 0.1)


            loss, logits = model(inputs,labels,scores,inputs_aug)
...
def evaluate(args, model, tokenizer,eval_when_training=False):
    ...
    print(tp,tn,fp,fn)
    eval_f1 = 2*p*r/(p+r)
    print(p,r,eval_f1,eval_acc)
    print(classification_report(labels, preds, digits=4))
    
    result = {
        "eval_loss": float(perplexity),
        "eval_acc":round(eval_f1,4),
        "eval_p":round(p,4)
    }
    return result
...
def test(args, model, tokenizer):
    ...
    for batch in tqdm(eval_dataloader,total=len(eval_dataloader)):
        ...
        score=batch[2].to(args.device)
        with torch.no_grad():
            logit = model(input_ids=inputs,score=score,input_ids_aug=inputs)
            logits.append(logit.cpu().numpy())
            labels.append(label.cpu().numpy())
        ...
        if args.teacher_path != '':
            if args.model_name_or_path:
                teacher_model = model_class.from_pretrained(args.model_name_or_path,
                                                    ...
                                                    config=config,
                                                    cache_dir=args.cache_dir if args.cache_dir else None)  
                teacher_model=Model(teacher_model,config,tokenizer,args)
            else:
                teacher_model = model_class(config)
            checkpoint_prefix = 'checkpoint-best-acc/model.bin'
            output_dir = os.path.join(args.teacher_path, '{}'.format(checkpoint_prefix))
            teacher_model.load_state_dict(torch.load(output_dir))
            teacher_model.to(args.device)
        train_dataset = TextDataset(tokenizer, args, args.train_data_file, teacher_model)
        if args.local_rank == 0:
