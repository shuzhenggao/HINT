...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        ...
        for step, batch in enumerate(bar):
            ...
            if aug_idx%4 == 0:
                inputs_aug[:, 2:], _ = mask_tokens(inputs.clone().detach()[:, 2:] ,tokenizer, 0.05)
            elif aug_idx%4 == 1:
                code_types = inputs.clone().detach()
                inputs_aug[:, 2:], _ = replace_with_type_tokens(inputs.clone().detach()[:, 2:], code_types.clone()[:, 2:],tokenizer, 0.05)
            elif aug_idx%4 == 2:
                choice_token_id  = choice(special_token_id_list)
                inputs_aug[:, 2:], _ = replace_special_token_with_type_tokens(inputs.clone().detach()[:, 2:], choice_token_id, tokenizer, 0.05)
            elif aug_idx%4 == 3:
                choice_token_id  = choice(special_token_id_list)
                inputs_aug[:, 2:], _ = replace_special_token_with_mask(inputs.clone().detach()[:, 2:], choice_token_id,tokenizer, 0.05)
...
def test(args, model, tokenizer):
    ...
    for batch in tqdm(eval_dataloader,total=len(eval_dataloader)):
        ...
        score=batch[2].to(args.device)
        with torch.no_grad():
            logit = model(input_ids=inputs,score=score)
            logits.append(logit.cpu().numpy())
            labels.append(label.cpu().numpy())
...
def main():
    ...
    parser.add_argument('--edit', type=float, default=0.2, help="For debugging.")
    parser.add_argument('--k', type=int, default=2, help="For debugging.")
