...
class Seq2Seq(nn.Module):
    ...
    def forward(self, source_ids, target_ids=None,score=None,args=None):   
        ...
        out_aug = self.decoder(target_ids,attention_mask=mask,past_key_values=encoder_output.past_key_values).last_hidden_state
        ...
        if 'pl_ours' in args.mode:
          ...
          if(args.submode=='ce'):
          ...
          elif(args.submode=='sce'):
              loss_fct = SCELoss(self.config,alpha=1, beta=1)
              loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1))[active_loss],
                      shift_labels.view(-1)[active_loss], score.view(-1)[active_loss])
          else:
            loss_fct = SCELoss(self.config,alpha=1, beta=1)
            loss0 = loss_fct(shift_logits.view(-1, shift_logits.size(-1))[active_loss],
                        shift_labels.view(-1)[active_loss], score.view(-1)[active_loss])
            lm_logits_aug = self.lm_head(out_aug)
            ...
            loss = loss0+loss_aug+args.p*kl_loss                    
        else:
          loss_fct = nn.CrossEntropyLoss(ignore_index=-1)
          loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1))[active_loss],
